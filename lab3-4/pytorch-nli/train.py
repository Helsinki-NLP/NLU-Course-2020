...
def main():
    ...
    for epoch in range(config.epochs):
        ...
        if config.optimizer == 'sgd':
            ...
            print(' (Early stopping countdown: {} epochs)'.format(config.early_stopping_patience-early_stopping))
        ...
        for batch_idx, batch in enumerate(train_iter):
            ...
            # sys.exit()
            # Calculate accuracy
            n_correct += (torch.max(answer, 1)[1].view(batch.label.size()).data == batch.label.data).sum().item()
            ...
            if stop_training == True or (1+epoch == config.epochs and 1+batch_idx == len(train_iter)):
                ...
                for test_batch_idx, test_batch in enumerate(test_iter):
                    ...
                    n_test_correct += (torch.max(answer, 1)[1].view(test_batch.label.size()).data == \
                        test_batch.label.data).sum().item()
